...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
  ...
  if (request.greeting == 'allData') {
    let beginDate = getDateRange(90);
    let endDate = getDateRange(2);
    ...
    chrome.tabs.query({
      ...
      $.ajax({
        url: `https://sycm.taobao.com/xsite/daren/single/text/all.json?dateRange=${beginDate}%7C${endDate}&dateType=range&pageSize=${allDataPageSize}&page=${allDataPage}&order=desc&orderBy=browsePv&keyword=&startDate=&endDate=&parentContentTypeId=1000&contentTypeId=&indexCode=&_=${generatTk(13)}&token=0c88cf3f1`,
                  ...
                  success: function(data) {
                    ...
                    $.ajax({
                      ...
                      success: function(data) {
                        ...
                        //非最后一面最后一条数据时，触发翻页事件
                        if (allDataPage < allDataTotalPage && ind == allDataPageSize - 1) {
                          allDataPage++;
